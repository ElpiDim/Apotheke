...
export function IntegrationPdfPage() {
  ...
  useEffect(() => {
    let objectUrl = '';
    void apiBlob(`/integrations/entries/${entryId}/pdf`)
      .then((blob) => {
  ...
  return (
    <div className={`fixed bottom-0 right-0 top-0 z-50 flex min-h-0 flex-col overflow-hidden bg-violet-950 ${fullscreen ? 'left-0' : 'left-0 sm:left-60'}`}>
      <header className="flex h-12 shrink-0 items-center gap-2 border-b border-violet-800 bg-violet-950 px-3 text-white sm:px-4">
        ...
        <a href={pdfUrl || undefined} download={entry.attachment?.originalFilename} aria-disabled={!pdfUrl} className="flex items-center gap-2 rounded-lg bg-coral-500 px-3 py-2 text-[11px] font-semibold text-white hover:bg-coral-600 aria-disabled:pointer-events-none aria-disabled:opacity-50"><Download size={14} /><span className="hidden sm:inline">Download</span></a>
      </header>
      {pdfUrl ? <iframe title={entry.title} src={`${pdfUrl}#view=FitH`} className="block h-0 min-h-0 w-full flex-1 border-0 bg-slate-700" /> : <div className="flex flex-1 items-center justify-center text-sm text-violet-300">Loading PDF…</div>}
    </div>
  );
}
